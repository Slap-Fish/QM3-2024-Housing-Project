...
# QM3 Group Project Markdown File

## Research Question

Here I will explain the research question.

Here I will outline the data.
...
# Remove rows of data from outsied London
...
All the summary stats for MonthlyRent_Mean is significantly larger than MonthlyRent_Median, suggesting the former is disproportionately affected by outliers, so it would probably be a good idea to use MonthlyRent_Median for our analysis.
...
For the next section, we were astonished by how much of a difference removing the outliers made in our regression analysis, so we run the analysis on the data before and after eliminating the outliers for comparison, but we only drew conclusions from data_no_outliers.
...
model <- lm(MonthlyRent_Mean ~ Job_Density, data = data_no_outliers)
model_with_outliers <- lm(MonthlyRent_Mean ~ Job_Density, data = data)
...
The regression model (from the estimated value and the p-value) shows that there is a strong and significant positive correlation between job density and rent prices in London boroughs.
...
No we consider homoscedasticity by plotting a Scale-Location plot. We want a straight line roughly following y=const, otherwise there is an implied relationship between the residuals and the fitted values, or a non-constant error variance.
...
This test comes our positive with a p-value \< 0.05, so this assumption is definitely violated.
...
The points labelled 17, 20 and 21 are very large and, as previously identified, are probably outliers. In further analysis we should remove these data points for a more precise picture of what's going on. However, we don't have many data points as it is, so it would be a good idea to further break up London boroughs into smaller areas so that we have more data points to work with, and can afford to eliminate all of the outliers.
...
However, it is beyond the scope of this assignment to run alternative models, so we leave our project here, with an acknowledgement that there is some relationship between the density of jobs in an area and the cost of renting in that area, but that this may or may not be best explained by a linear correlation. There is more analysis to be done to truky understand what's going on here.

### Bonus: Geospacial Analysis

We did also run some tests to see if geospacial trends are present, but since these were not done in RStudio we shall not discuss them here. However, this analysis is outlined in our video presentation.
